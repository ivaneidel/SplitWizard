...
function GroupRow({ g }: { g: Group }) {
  return (
    <Link
      to={`/groups/${g.id}`}
      className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white p-4 transition hover:border-indigo-300 dark:border-zinc-700 dark:bg-zinc-800 dark:hover:border-indigo-600"
    >
      {g.photoURL ? (
        <img
          src={g.photoURL}
        ...
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300">
          <Users size={18} />
        </span>
      )}
...
export function Dashboard() {
  ...
  return (
    <div className="space-y-4">
      ...
      {expensesLoading && monthEntries.length === 0 && (
